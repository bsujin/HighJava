...
public class C_ThreadTest15 {

	public static void main(String[] args) {
		// 공통으로 사용할 객체 생성
		ShareData sd = new ShareData();
		
		// 쓰레드 객체를 생성하고, 공통으로 사용할 객체를 ㄹ쓰레드에 주입한다.
	...
	public void run() {
		...
		for(int i=1; i<=1000000000; i+=2) {
			if(	(i/2) % 2 ==0 ) {	// 2로 나눈 몫이 짝수이면...
				sum += 1.0/i;   //1로 나누면 정수라서 1.0으로 해야됨
			}else {
				sum -= (1.0 / i);
			}
		}
...
// 공통으로 사용할 클래스 ===> 원주율 관리하는 클래스

class ShareData{
	public double result;	//계산된 원주율이 저장될 변수 
	public volatile boolean isOk;	//계산이 완료되었는지 여부를 나타내는 변수

	/*	volatile  : 무조건 메모리에서 읽어와라 (캐시를 거치지말고)
		이 키워드가 붙은 변수는 컴파일러의 최적화 대상에서 제외된다.
